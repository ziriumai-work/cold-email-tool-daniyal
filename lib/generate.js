...
9. SIGN-OFF: Do NOT include a signature, sign-off, sender name, role, or contact info (e.g., no "Best regards,", no "Name, Role, Zirium AI"). End the email body immediately after the CTA. The signature will be automatically attached at send time.

OUTPUT FORMAT (STRICT — return ONLY the subject line and email body, no preambles or notes):
Subject: <under 8 word subject>

<Email body starting with Hi [Name], or Hi [Company Name], and ending cleanly after the CTA without any sign-off or signature>`;

function customWriterPrompt(company, sender, customPrompt) {
  let domainContext = '';
  if (company.website || company.domain) {
    const rawSite = company.website || company.domain || '';
    const cleanHost = String(rawSite).replace(/^https?:\/\//i, '').replace(/^www\./i, '').split('/')[0];
    domainContext = `WEBSITE / DOMAIN: ${cleanHost}`;
  }

  let contactName = (company.contact_name || '').trim();
  if (!contactName && company.contact_email && company.contact_email.includes('@')) {
    const local = company.contact_email.split('@')[0].replace(/[\._\-\+]+/g, ' ').trim();
    if (local && !['info', 'contact', 'admin', 'sales', 'hello', 'support', 'team', 'office'].includes(local.toLowerCase())) {
      contactName = local.split(' ').map((w) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');
    }
  }

  let contextDetails = `TARGET COMPANY: ${company.name || 'Target Company'}`;
  contextDetails += `\nCONTACT PERSON: ${contactName || 'Founder / Business Lead'}`;
  ...
  // Derive dynamic company-specific angle focus to guarantee distinct email lenses
  const seedNum = (String(company.id || company.name || '').split('').reduce((acc, c) => acc + c.charCodeAt(0), 0)) % 5 + 1;
  const angleFocusMap = {
    1: 'Focus on scaling operational efficiency, reducing manual workload, and streamlining core team workflows.',
    2: 'Focus on automated lead qualification, faster inbound response times, and routing high-value prospects.',
    3: 'Focus on custom AI agentic systems that handle repetitive reporting, tracking, and operational tasks.',
    4: 'Focus on bespoke AI integration tailored specifically to how their existing business operations run.',
    5: 'Focus on unlocking team bandwidth and accelerating business throughput without adding headcount.',
  };
  const dynamicAngle = angleFocusMap[seedNum] || angleFocusMap[1];
  ...
  return [
    { role: 'system', content: CUSTOM_WRITER_SYSTEM },
    {
      ...
      content:
        ...
        `TAILORED OPERATIONAL ANGLE FOR ${company.name || 'THIS LEAD'}:\n${dynamicAngle}\n\n` +
        `STRICT UNIQUNESS REQUIREMENT:\n` +
        `This email is for ${company.name || 'this company'}. Make the subject line and opening sentence completely distinct and personalized to ${company.name}. Do NOT write a generic cookie-cutter template or use identical phrasing from other emails in the campaign.\n\n` +
        `Draft the cold outreach email following all instructions and rules. Return strictly in the required format without any sign-off or signature block.`,
    },
  ];
}

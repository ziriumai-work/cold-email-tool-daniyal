...
2. SUBJECT LINE: Short (under 8 words), specific to the industry/offer, zero hype words ("revolutionize", "unlock", "game-changing"). Do NOT include the target company's name in the subject line.
3. OPENING LINE: Reference the core message/offer directly. Never open with generic fillers like "I hope this finds you well" or "My name is".
...
5. THE OFFER: Connect their specific pain point directly to a custom AI system Zirium could build (e.g., "an agent that qualifies and routes inbound leads before your team sees them"). Frame around their problem, not our product.
...
8. LENGTH & FORMAT: Retain standard email length (250–400 words total, fully developed outreach). Paragraphs strictly max 4 lines long.
9. BANNED ELEMENTS: No exclamation points, no emojis, no markdown formatting (no bold/italics), no buzzwords ("synergy", "leverage", "cutting-edge", "seamless", "transform", "skyrocket", "supercharge"). NEVER include or mention the company name, contact name, or brand name anywhere in the subject line or email body.
...
function customWriterPrompt(company, sender, customPrompt) {
  ...
  let contextDetails = '';
  if (company.industry) {
    contextDetails += `INDUSTRY: ${company.industry}\n`;
  }
  if (company.notes) {
    contextDetails += `PAIN POINT / RECENT CONTEXT: ${company.notes}\n`;
  }
  ...
  return [
    { role: 'system', content: CUSTOM_WRITER_SYSTEM },
    {
      ...
      content:
        ...
        `TAILORED OPERATIONAL ANGLE:\n${dynamicAngle}\n\n` +
        `STRICT NO-COMPANY-NAME REQUIREMENT:\n` +
        `Do NOT mention or include any company name, contact person name, or brand name anywhere in the subject line or email body. Keep the copy completely generic of specific company names.\n\n` +

...
export function normalizeKey(h) {
  ...
  if (['company', 'companyname', 'name', 'business', 'businessname', 'organization', 'org'].includes(k)) return 'name';
  if (['website', 'url', 'site', 'web', 'domain', 'link', 'webpage'].includes(k)) return 'website';
  if (['email', 'contactemail', 'contact', 'mail', 'emailaddress', 'primaryemail'].includes(k)) return 'contact_email';
  if (['phone', 'phonenumber', 'number', 'contactnumber', 'tel', 'telephone', 'mobile', 'cell'].includes(k)) return 'phone';
  return k;
}

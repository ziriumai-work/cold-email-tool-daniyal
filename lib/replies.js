...
function extractSnippet(msg) {
  ...
  for (const sender of accounts) {
    ...
    try {
      ...
      if (uids && uids.length) {
        ...
        for (let i = 0; i < uids.length; i += batchSize) {
          ...
          try {
            for await (const msg of client.fetch(rangeStr, { uid: true, envelope: true, source: true, headers: ['in-reply-to', 'references'] }, { uid: true })) {
    ...
    // Process fetched messages safely after IMAP connection is closed

...
export async function checkReplies({ days = 21 } = {}) {
  ...
  const existingReplies = await all('SELECT imap_message_id, snippet FROM replies WHERE imap_message_id IS NOT NULL');
  const known = new Set(existingReplies.map((r) => r.imap_message_id));
  const emptySnippetIds = new Set(existingReplies.filter((r) => !r.snippet || r.snippet.trim() === '').map((r) => r.imap_message_id));
...
function extractSnippet(msg) {
  try {
    ...
    if (body.includes('Content-Type: text/plain')) {
      const parts = body.split(/Content-Type:\s*text\/plain/i);
      body = parts[parts.length - 1] || body;
      const headerEnd = body.search(/\r?\n\r?\n/);
      if (headerEnd !== -1) body = body.slice(headerEnd + 4);
    } else if (body.includes('Content-Type: text/html')) {
      const parts = body.split(/Content-Type:\s*text\/html/i);
      body = parts[parts.length - 1] || body;
      const headerEnd = body.search(/\r?\n\r?\n/);
      if (headerEnd !== -1) body = body.slice(headerEnd + 4);
    } else {
    ...
    if (body.includes('\r\n--')) {
      const subParts = body.split(/\r?\n--/);
      body = subParts[0] || body;
    }

    if (body.includes('Content-Transfer-Encoding: quoted-printable') || /=[\r\n]/.test(body) || /=[0-9A-F]{2}/i.test(body)) {
  ...
  for (const sender of accounts) {
    ...
    try {
      ...
      if (uids && uids.length) {
        ...
        for (let i = 0; i < uids.length; i += batchSize) {
          ...
          try {
            for await (const msg of client.fetch(rangeStr, { uid: true, source: true, envelope: true, headers: ['in-reply-to', 'references'] }, { uid: true })) {
              const env = msg.envelope || {};
              const imapMsgId = env.messageId || `${sender.email}:uid-${msg.uid}`;

              const isExistingEmpty = emptySnippetIds.has(imapMsgId);
              if (known.has(imapMsgId) && !isExistingEmpty) continue;
              ...
              fetchedMsgs.push({ msg, match, env, fromAddr, subject, received, imapMsgId, isExistingEmpty });
            }
          } catch (batchErr) {
    ...
    for (const item of fetchedMsgs) {
      const { msg, match, fromAddr, subject, received, imapMsgId, isExistingEmpty } = item;
      const receivedSql = received ? received.toISOString().slice(0, 19).replace('T', ' ') : null;
      const snippetText = extractSnippet(msg);
      try {
        if (isExistingEmpty) {
          if (snippetText) {
            const { classifyReplyIntent } = await import('./aiReply.js');
            const intent = await classifyReplyIntent(subject, snippetText);
            const replyStatus = (intent.category || 'new').toLowerCase();
            await run('UPDATE replies SET snippet = ?, status = ?, notes = ? WHERE imap_message_id = ?', [snippetText, replyStatus, intent.recommendedAction || null, imapMsgId]);
            emptySnippetIds.delete(imapMsgId);
            found++;
          }
          continue;
        }

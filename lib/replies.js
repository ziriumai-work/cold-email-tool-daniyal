...
function extractSnippet(msg) {
  ...
  for (const sender of accounts) {
    ...
    try {
      ...
      if (uids && uids.length) {
        const fetchRange = uids.length === 1 ? String(uids[0]) : `${uids[0]}:${uids[uids.length - 1]}`;
        for await (const msg of client.fetch(fetchRange, { uid: true, envelope: true, source: true, headers: ['in-reply-to', 'references'] }, { uid: true })) {

...
export async function unsuppressEmail(email) {
  ...
  try {
    const cleanEmail = email.toLowerCase().trim();
    await db.run('DELETE FROM suppression_list WHERE lower(email) = ?', [cleanEmail]);
    return true;
  } catch (err) {
    console.error('Error removing email from suppression list:', err.message);
    return false;
  }
}
...
export function appendComplianceFooter(htmlContent, recipientEmail, trackingId) {
  ...
  // For 1-to-1 ls, heavy HTML footers with 'CAN-SPAM' text and borders trigger Gmail's Promotions tab.

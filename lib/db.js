...
// Check if database error is due to a missing table
...
async function handleSelect(client, sql, args) {
  ...
  if (whereMatch) {
    ...
    for (const cond of conditions) {
      ...
      if (/(?:lower\()?([a-zA-Z0-9_]+\.)?(\w+)\)?\s+in\s*\(([^)]+)\)/i.test(trimmed)) {
        const inMatch = trimmed.match(/(?:lower\()?([a-zA-Z0-9_]+\.)?(\w+)\)?\s+in\s*\(([^)]+)\)/i);
        const col = inMatch[2];
        const statuses = inMatch[3].split(',').map((s) => s.trim().replace(/'/g, ''));
        query = query.in(col, statuses);
      } else if (/(?:lower\()?([a-zA-Z0-9_]+\.)?(\w+)\)?\s+is\s+not\s+null/i.test(trimmed)) {
        const nullMatch = trimmed.match(/(?:lower\()?([a-zA-Z0-9_]+\.)?(\w+)\)?\s+is\s+not\s+null/i);
        query = query.not(nullMatch[2], 'is', null);
      } else if (/(?:lower\()?([a-zA-Z0-9_]+\.)?(\w+)\)?\s*=\s*\?/i.test(trimmed)) {
        const eqMatch = trimmed.match(/(?:lower\()?([a-zA-Z0-9_]+\.)?(\w+)\)?\s*=\s*\?/i);
        ...
        const eqLitMatch = trimmed.match(/(?:lower\()?([a-zA-Z0-9_]+\.)?(\w+)\)?\s*=\s*'([^']+)'/i);
        query = query.eq(eqLitMatch[2], eqLitMatch[3]);
      } else if (/([a-zA-Z0-9_]+\.)?(\w+)\s*=\s*(\d+)/i.test(trimmed)) {
        const numLitMatch = trimmed.match(/([a-zA-Z0-9_]+\.)?(\w+)\s*=\s*(\d+)/i);
        query = query.eq(numLitMatch[2], Number(numLitMatch[3]));
      }
    }
  }
  ...
  if (orderMatch) {
    ...
    for (const part of orderParts) {
      const match = part.match(/^(?:([a-zA-Z0-9_]+)\.)?(\w+)(?:\s+(asc|desc))?$/i);
      if (match) {
        query = query.order(match[2], { ascending: (match[3] || 'asc').toLowerCase() === 'asc' });
      }
    }
  }
...
async function handleUpdate(client, sql, args) {
  ...
  for (let i = 0; i < setStr.length; i++) {
    ...
    if (char === ',' && depth === 0) {
      if (currentPart.trim()) setParts.push(currentPart.trim());
      currentPart = '';
    } else {
      currentPart += char;
    }

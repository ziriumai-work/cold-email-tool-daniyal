...
export async function sendDueScheduled() {
  ...
  for (const d of due) {
    ...
    try {
      ...
      console.log(`[scheduler] sent draft ${d.id} to ${company.contact_email}`);

      // Enterprise Branching Sequence: Process next sequence step for company
      try {
        const { processSequenceStepForCompany } = await import('./sequences.js');
        await processSequenceStepForCompany(d.company_id, { ...d, status: 'sent', open_count: d.open_count || 0 });
      } catch (seqErr) {
        console.error('[scheduler] sequence step error:', seqErr.message);
      }
    } catch (e) {

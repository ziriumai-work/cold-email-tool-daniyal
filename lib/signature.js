...
export async function signatureData(sender = {}) {
  ...
  const [name, title, tagline, website, logo, calendly] = await Promise.all([
  ...
  return { name: name || '', title: title || '', tagline: tagline || '', website: website || '', logo: logo === '1', calendly: calendly || '' };
}

export function signatureText(s) {
  return [s.name, s.title, s.tagline, s.website, s.calendly ? `Schedule meeting: ${s.calendly}` : ''].filter(Boolean).join('\n');
}

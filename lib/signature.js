...
export function defaultSignature(sender = {}) {
  ...
  if (key === 'wahaj' || email === 'wahaj.s@ziriumai.com') {
    return { name: 'Wahaj Shehzad', title: 'Managing Director', tagline: 'ZiriumAI', website: 'ziriumai.com', logo: true };
  }
  if (key === 'info' || email === 'info@ziriumai.com') {
    return { name: 'ZiriumAI', title: '', tagline: '', website: 'ziriumai.com', logo: true };
  }
  if (key === 'haseeb' || email === 'haseeb.a@ziriumai.com') {
    return { name: 'Haseeb Akbari', title: 'CTO', tagline: 'ZiriumAI', website: 'ziriumai.com', logo: true };
  }
  return { name: sender.name || '', title: '', tagline: 'ZiriumAI', website: 'ziriumai.com', logo: true };
}
...
export async function signatureData(sender = {}) {
  ...
  const [name, title, tagline, website, logo] = await Promise.all([
  ...
  return { name: name || '', title: title || '', tagline: tagline || '', website: website || '', logo: logo === '1' };
}

export function signatureText(s) {
  return [s.name, s.title, s.tagline, s.website].filter(Boolean).join('\n');
}
...
export function signatureHtml(s) {
  if (!s.name && !s.title && !s.tagline && !s.website && !s.logo) return '';
